/*
  Jose Salazar, Russell Price, Bryan Goforth, Sara Warren
  CSCE4600 Project 2
...
void ReadFromFile(string userfile)
{
  ...
  if(inputfile.is_open())
  {
    ...
    while(getline(inputfile,line))
    {
      ...
      //we check to make sure the process names are no more than 20 characters
      ...
      else  //processes with names larger than 20 characters will be displayed below for the user
      {
...
int Pivot(int First, int Last)
{
  ...
  {
    if(SCHEDULE_KEY==0)//SJF SPECIFIC
    {
    ...
    if(SCHEDULE_KEY==1)//PRI SPECIFIC
    {
  ...
  if(SCHEDULE_KEY==0)//SJF SPECIFIC
  {
    ...
    SwapS(SJF_ProcessID[p],SJF_ProcessID[First]);  //changes the ProcessID ordering
  ...
  if (SCHEDULE_KEY==1)//PRI SPECIFIC
  {
    ...
    SwapS(PRI_ProcessID[p], PRI_ProcessID[First]); //changes the ProcessID with RunTime swap (strings)
...
void Calculate_anyStart(int anyStart[50], int anyRunTime[50])
{
    //calculate start time for x number of processes.
    //starting processes have 0 since they start.
    //next processes start time receive next value accourding to s-1 value
...
void Calculate_anyTurnAround(int anyTurn[50], int anyEndTime[50], int anyArrival[50], int& anyAVG)
{
    //for x number of processes perfrom TurnaroundTime = EndTime - arrivalTime
    //calculate average
   for(int t=0; t < x; t++)
  {
...
void Calculate_anyWaitTime(int anyWait[50], int anyStart[50], int anyArrival[50], int& anyAVG)
{
    //for total number of processes calculate waiting time = startTime + arrivalTime
    //calculate average
   for(int w=0; w < x; w++)
  {
...
void Calculate_Res(int anyRes[50], int anyStart[50], int& anyAVG)
{
    //for total number of processes calculate responce time = starttime
    //average the responce times
   for(int r=0; r < x; r++)
  {
    anyRes[r]=anyStart[r];
    anyAVG+=anyRes[r];
...
void Display(string PID[50], int A[50], int R[50],int P[50], int S[50], int E[50], int T[50],int W[50], int Res[50],int& ATA, int& AWT, int& ART, int& C)
{
  ...
  //perform quick average calculations and display.
